...
export function sumPoints(questions: Question[]): number {
    return questions.reduce((sum: number, question) => sum + question.points, 0);
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.filter(question => question.published)
                    .reduce((sum: number, question) => sum + question.points, 0);
}
...
export function makeAnswers(questions: Question[]): Answer[] {
    return questions.map(question => ({questionId: question.id, text: "", submitted: false, correct: false}));
}
    ...
    return questions.map(question => 
        question.id !== targetId ? question :
        {
            ...
            options: targetOptionIndex === -1 ? [...question.options, newOption] :
                question.options.map((option: string, index: number) => index === targetOptionIndex ? newOption : option)
        }
    );
}
...
export function duplicateQuestionInArray(
    ...
    // if (index === -1) return questions;   // Test cases never cover this possibility
